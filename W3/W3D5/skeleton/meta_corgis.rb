...
class MetaCorgiSnacks
  ...
  #   result = name.to_s.capitalize + ": " + @snack_box.send("get_#{name}_info", @box_id) + ": " + @snack_box.send("get_#{name}_tastiness", @box_id).to_s
  # tastiness > 30 ? "* #{result}" : result
  # end
  ...
  def self.define_snack(name)
    define_method(:name) do
      ...
      name_to_display = "#{name.split('_').map(&:capitalize).join(' ')}"
      result = "#{name_to_display}: #{info}: #{tastiness}"
      tastiness > 30 ? "* #{result}" : result
    end
  end
end
